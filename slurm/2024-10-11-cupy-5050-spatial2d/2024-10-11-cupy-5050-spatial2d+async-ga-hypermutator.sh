...
WSE_SKETCHES_REVISION="18c089f74aef45bbeac97a661b71f7d94c30a350"
echo "WSE_SKETCHES_REVISION ${WSE_SKETCHES_REVISION}"

WORKDIR="${HOME}/scratch/2024-10-11-cupy-5050-spatial2d/nben=${NBEN}+async-ga-hypermutator-512"
...
seed=0
export ASYNC_GA_NCOL=243
echo "ASYNC_GA_NCOL ${ASYNC_GA_NCOL}"

export ASYNC_GA_NROW=243
...
for config in \
    ...
    "export ASYNC_GA_NCOL_SUBGRID=243 ASYNC_GA_NROW_SUBGRID=243 NREP=9" \
; do
eval "${config}"
echo "ASYNC_GA_NCOL ${ASYNC_GA_NCOL}"
echo "ASYNC_GA_NROW ${ASYNC_GA_NROW}"
...
echo "python3.10 \$(which python3.10)"
echo "python3.10 --version \$(python3.10 --version)"
MODULE_PATH="/opt/software-current/2023.06/x86_64/generic/software/CUDA/12.3.0/"
echo "MODULE_PATH \${MODULE_PATH}"
export LD_LIBRARY_PATH="\${MODULE_PATH}/lib64:\${LD_LIBRARY_PATH}"
...
def get_gpu_info():
    if cp.cuda.is_available():
        ...
        print(f"GPU Memory: {cp.cuda.runtime.getDeviceProperties(0)['totalGlobalMem'] // 1024 ** 2} MB")
    else:
...
./pyscript/hypermutator-5050-spatial2d.py
